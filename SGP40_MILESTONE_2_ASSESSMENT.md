...
I2 reconciliation addendum (2026-08-14): the SGP40 driver is now integrated
with the I1 sensor set at fixed `POD_1` slot 3. Its only compensation authority
is a recent valid SHT41 snapshot; no 25 C/50% RH fallback is composed. Historical
local-alarm/output statements are superseded: I2 has no alarm state machine or
physical output driver, and no physical SGP40 validation is claimed.
...
| VOC algorithm cadence | 1 second | `CONFIG_SITETWIN_SGP40_ALGORITHM_INTERVAL_1S` |
| Optional cadence | 10 seconds | `CONFIG_SITETWIN_SGP40_ALGORITHM_INTERVAL_10S` |
| Maximum compensation age | 2000 ms | `CONFIG_SITETWIN_SGP40_COMPENSATION_MAXIMUM_AGE_MS` |
...
- Alarm/control behavior is deliberately absent from I2 and remains a later control-layer workstream.
