...
{
    if (error == ESP_ERR_INVALID_STATE && device != NULL && device->bus_handle != NULL) {
        /*
         ...
         */
        esp_err_t probe_result = i2c_master_probe(device->bus_handle,
                                    ...
                                    const st_espidf_i2c_device_config_t *config)
{
    ...
    if (device == NULL || config == NULL || config->controller < 0 ||
        config->sda_gpio < 0 || config->scl_gpio < 0 || config->address > 0x7FU ||
    ...
    result = i2c_new_master_bus(&bus_config, &device->bus_handle);
    if (result != ESP_OK) {
        memset(device, 0, sizeof(*device));
        return result;
    }
    ...
    result = i2c_master_bus_add_device(device->bus_handle, &device_config,
                                       &device->device_handle);
    if (result != ESP_OK) {
        (void)i2c_del_master_bus(device->bus_handle);
        memset(device, 0, sizeof(*device));
    }
    return result;
}
